...
from
	...
	left join Production.erp.PartTran pt							--for KB jobs
		on (pt.TranDate >= dateadd(d,-3,ld.PayrollDate) or pt.TranDate <= dateadd(d,3,ld.PayrollDate) )
...
select
	...
	1 QuantityContinue,
	'true' JobComplete,
	 CONVERT(varchar, getdate(), 23) JobCompletionDate,
	'true' JobClosed,
	 CONVERT(varchar, getdate(), 23) ClosedDate
